...
Registers `multi_tool_use_seq_dependent`, a composite tool for dependent tool execution.

Behavior:
- executes listed tool calls in order
- stops on the first tool error
- returns completed step outputs to the model
- uses `ctx.runTool(...)` to invoke other active tools through pi's normal core dispatch path
...
```bash
cd dependent_multi_tool_corehook
