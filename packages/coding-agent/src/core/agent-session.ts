...
export class AgentSession {
	...
	private _installAgentToolHooks(): void {
		const beforeToolCall = async ({ toolCall, args }: { toolCall: ToolCall; args: unknown }) => {
		...
		const hookAgent = this.agent as Agent & {
			setBeforeToolCall?: (handler: typeof beforeToolCall) => void;
			setAfterToolCall?: (handler: typeof afterToolCall) => void;
		};

		if (typeof hookAgent.setBeforeToolCall === "function") {
			hookAgent.setBeforeToolCall(beforeToolCall);
		} else {
			this.agent.beforeToolCall = beforeToolCall;
		}

		if (typeof hookAgent.setAfterToolCall === "function") {
			hookAgent.setAfterToolCall(afterToolCall);
		} else {
			this.agent.afterToolCall = afterToolCall;
		}
	}
